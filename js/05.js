...
async function getRandomTodos() {
  const { data } = await axios.get(`${BASE_URL}/todos/random`);
  console.log(data);
  return data;
}
// getRandomTodos();
...
async function getProductsCategories() {
  ...
  const categories = data.map((prop) => {
    return prop["name"];
  });
...
async function getRandomQutoe() {
  const { data } = await axios.get(`${BASE_URL}/quotes/random`);
  console.log(data);
  return data;
}
// getRandomQutoe();
...
async function getRecipeByTag(tag) {
  const { data } = await axios.get(`${BASE_URL}/recipes/tag/${tag}`);
  console.log(data);
  return data;
}
// getRecipeByTag("Beef");
...
async function getCartByUserId(id) {
  const { data } = await axios.get(`${BASE_URL}/carts/user/${id}`);
  console.log(data);
  return data;
}
getCartByUserId(6);
