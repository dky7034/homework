...
todosWithPriorityLabels = originTodos.map((element) => {
  switch (element["priority"]) {
    case HIGH_PRIORITY:
      return {
        ...element,
        text: "[긴급] " + element["text"],
      };
    case MEDIUM_PRIORITY:
      return {
        ...element,
        text: "[보통] " + element["text"],
      };
    case LOW_PRIORITY:
      return {
        ...element,
        text: "[낮음] " + element["text"],
      };
    default:
      return element;
  }
...
/* 로직 코드 작성 */
allTodosCompleted = originTodos.every((element) => {
...
/* 로직 코드 작성 */
hasUrgentTodos = originTodos.some((element) => {
  if (element["priority"] === HIGH_PRIORITY && element["completed"] === false) {
    return true;
  }
});
// 출력 코드
...
/* 로직 코드 작성 */
completedCount = originTodos.reduce((acc, element, index) => {
  if (element["completed"] === true) {
    acc++;
  }
  return acc;
...
importantIncompleteTodos = originTodos.filter((element) => {
  if (element["completed"] === IS_NOT_COMPLETED) {
    if (
      ...
      element["priority"] === HIGH_PRIORITY
    ) {
      return true;
    }
  }
